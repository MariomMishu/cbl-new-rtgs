...
public class RejectedSISpecification {
    ...
    public static Specification<SiUpcomingItem> hasNotUser(Long userId) {
        return (root, query, criteriaBuilder) -> criteriaBuilder.notEqual(root.get("rejecter").get("id"), userId);
    }
    ...
    public static Specification<SiUpcomingItem> hasExpiryDate(String expiryDate) {
        return (root, query, criteriaBuilder) -> criteriaBuilder.equal(root.<LocalDate>get("siConfiguration").get("expiryDate"), SiUtility.toExpiryDate(expiryDate));
    }
